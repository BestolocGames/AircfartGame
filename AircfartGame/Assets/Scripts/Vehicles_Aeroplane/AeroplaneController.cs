// dnSpy decompiler from Assembly-CSharp.dll class: UnityStandardAssets.Vehicles.Aeroplane.AeroplaneController
using System;
using UnityEngine;
...
namespace UnityStandardAssets.Vehicles.Aeroplane
{
	...
	public class AeroplaneController : MonoBehaviour
	{
		...
		public float AerodynamicEffect
		{
			get
			{
				return this.m_AerodynamicEffect;
			}
			set
			{
				this.m_AerodynamicEffect = value;
			}
		}
		...
		{
			if (this.m_Rigidbody.velocity.sqrMagnitude > this.m_MaxSpeed * this.m_MaxSpeed)
			{
				this.m_Rigidbody.velocity = this.m_Rigidbody.velocity.normalized * this.m_MaxSpeed;
			}
		}
		...
		private void CaluclateAerodynamicEffect()
		{
			if (this.m_Rigidbody.velocity.magnitude > 0f)
			{
				...
				if ((double)this.m_Rigidbody.velocity.sqrMagnitude > (double)(this.MaxSpeed * this.MaxSpeed) * 0.01)
				{
					Quaternion b = Quaternion.LookRotation(this.m_Rigidbody.velocity, base.transform.up);
					this.m_Rigidbody.rotation = Quaternion.Slerp(this.m_Rigidbody.rotation, b, this.m_AerodynamicEffect * Time.deltaTime);
				}
			}
		}
